/*
Voici un exemple de tableau de personnes. Écris une fonction qui à partir d'un tableau 
similaire reçu en paramètre renverra un nouveau tableau,
...
function filterSeniors(persons) {
  const devSeniors = persons.filter(
    (person) => person.job === "dev" && person.experience >= 5
  );

  const dataSeniors = persons.filter(
    (person) => person.job === "data" && person.experience >= 5
  );

  return [devSeniors, dataSeniors];
}
const seniors = filterSeniors(persons);
console.log(seniors);
